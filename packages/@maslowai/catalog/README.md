...
[![npm version](https://badge.fury.io/js/%40roster%2Fcatalog.svg)](https://badge.fury.io/js/%40roster%2Fcatalog)
...
This package provides a comprehensive catalog of ready-to-use prompts covering various domains including development, documentation, database optimization, and MCP server setup. All prompts are type-safe and validated against Roster MCP-oriented contracts in this repo.
...
```bash
npm install @maslowai/catalog
# or
pnpm add @maslowai/catalog
# or
yarn add @maslowai/catalog
```
...
```typescript
import catalog from '@maslowai/catalog';
...
```typescript
import { catalog, type PromptsCatalog } from '@maslowai/catalog';
import type { Prompt } from '@maslowai/contracts';
...
const sqlPrompts = allPrompts.filter((p) => p.tags?.includes('sql') || p.category === 'Database');
```
...
- [`@maslowai/contracts`](../contracts) - Type definitions and schemas
- Core business logic lives in the main `@maslowai/roster` package under `src/core/`
- Storage adapters live under `src/adapters/` in `@maslowai/roster`
