...
program
  .name('roster')
  .description('Roster CLI (MCP prompts server) with optional AWS integration')
  .version('3.14.0');
    ...
    try {
      ...
      const promptData = {
        ...
        variables: options.variables
          ? options.variables.split(',').map((v: string) => v.trim())
          : ([] as string[]),
      };
...
program
  ...
  .option(
    '-r, --repo <repo>',
    'GitHub repository URL for catalog operations (set when using remote catalog)',
    '',
  )
  .action(async (options) => {
    try {
...
program
  .command('start')
  .description('Start the Roster MCP server')
